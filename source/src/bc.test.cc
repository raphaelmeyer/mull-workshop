...
TEST_SUITE("definition of a single block definition")
{
  TEST_CASE("a block contains a hash")
  {
    Block b{"", {}, {}};
    REQUIRE(not b.hash().empty());
  }

  TEST_CASE("a block stores some kind of data")
  {
    Block b{"Hello World", {}, {}};
    REQUIRE(b.content() == "Hello World");
  }

  TEST_CASE("a block has a timestamp")
  {
    auto const now = std::chrono::system_clock::now();
    Block b{"", now, {}};
    REQUIRE(b.timestamp() == now);
  }

  TEST_CASE("a block stores the hash of the previous block")
  {
    Hash previous{};
    std::fill(previous.begin(), previous.end(), 0x2A);
    Block b{"", {}, previous};
    REQUIRE(b.previous() == previous);
  }
}
